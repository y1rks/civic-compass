...
### `match_score` の絶対値は記事数・議員数で動く（未対処）

**順位ではなく％そのものが動きます。** UI に「マッチ度 62%」と出す以上、記事や議員を
足した時点で数字が変わることを承知しておいてください。

**① 回答したセルが増えるほど全員のマッチ度が上がる**

```
  3セル  1位 61  中央 44  最下位 21
 15セル  1位 75  中央 66  最下位 43
 60セル  1位 99  中央 73  最下位 58     ← 最下位が21→58。差が潰れる
```

分母 `Σ u.share` は平滑化の性質上**常に 1.0** になる一方、分子の
`Σ sqrt(u.share × p.share)`（Bhattacharyya係数）は重なるセルが増えるほど大きくなるためです。
記事が8本＝最大15セルの現状では顕在化していません。

```
セル数 |Σsqrt(u·p)| ×log(1+d) | silent項 | 合計
    3 |    0.484 |    0.429 |   0.186 | 0.614
   60 |    0.920 |    0.708 |   0.300 | 1.008   ← 100%を超える
```
...
倍率が尖り、`log(1 + distinctiveness)` を通じて分子が膨らみます。設計どおりの挙動ですが、
％の意味が議員リストに依存することになります。

直すなら、分子を Bhattacharyya 係数の上限（1.0）で正規化する、`log(1 + distinctiveness)` を
平均1になるよう規格化する、といった手があります。**`USER_SHARE_PRIOR` とは無関係**なので、
そちらを触っても解決しません。
